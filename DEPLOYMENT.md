...
This is a polyglot monorepo. The four packages deploy independently, each from
...
- Root / base directory: `landing/`
- Build command: none
- Publish directory: `landing/` (serves `index.html`)
...
matching subfolder (`backend/`, `frontend/`, `landing/`). Until that is done, the
live site is frozen at its last pre-migration build rather than broken.
...
suite, the frontend type-checks/builds/tests, the extension compiles/lints/tests,
and the landing page is validated as HTML.
