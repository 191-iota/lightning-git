...
![VS Code](https://img.shields.io/badge/VS%20Code-extension-1a1a1a?style=flat-square&logo=visualstudiocode&logoColor=white) ![TypeScript](https://img.shields.io/badge/TypeScript-1a1a1a?style=flat-square&logo=typescript&logoColor=white) ![live](https://img.shields.io/badge/live-lightning--git.com-9b2c2c?style=flat-square)
...
- [lightning-git-backend](https://github.com/191-iota/lightning-git-backend) — the Rust/actix-web server that owns the read-only repo clones, the in-RAM overlay state, the WebSocket realtime layer, and merge-conflict prediction.
- [lightning-git-frontend](https://github.com/191-iota/lightning-git-frontend) — the Vue 3 web dashboard for non-coding stakeholders: Kanban, the live overlay view, org and project management.
...
Press F5 in VS Code to launch an Extension Development Host. Point `lightningGit.apiUrl` and `lightningGit.wsUrl` at a running [backend](https://github.com/191-iota/lightning-git-backend) (defaults assume `localhost:8080`). You will need an account — register from the command palette — and the workspace must be a git checkout whose `origin` matches a project that already exists in the web frontend.
...
For the why behind the design, and for the conflict-prediction algorithm in full, see the [backend](https://github.com/191-iota/lightning-git-backend) and [lightning-git.com](https://lightning-git.com).
