...
Start a session and the extension begins watching your active editor. As teammates type on the same file from their own machines, their changes arrive over a WebSocket. Each teammate gets a stable colour, and their changed lines are tinted in that colour with their display name on the first one, so you see who is editing where at a glance instead of a wall of stringified content. Hovering a tinted line shows that teammate's version of it, syntax-highlighted, with a one-click link to open the full side-by-side diff (`Lightning Git: View Teammate Change`). A status bar item on the right shows `N editing` and flashes a warning background each time a teammate's keystrokes land, so activity is visible even when you are not looking at the decorated lines.

Predicted merge conflicts paint in the editor before anyone commits. When two people change the same lines in incompatible ways, those line ranges get an orange left border and an overview-ruler tick, and the first line of each region carries an inline badge naming the branches that disagree (`⚠ conflict: main ↔ feature/x`), so you see what the conflict is without leaving the file. Hovering shows each side's version syntax-highlighted, a second status bar item shows `N conflict(s)`, and the conflict panel lists every region with contributor chips coloured to match the in-editor tints. A forming conflict shows up while it is still cheap to talk about, hours before either branch reaches a pull request.
